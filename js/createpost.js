...
$(document).ready(function(){
	...
	activePostHandler();
	inactivePostHandler();
	updateCallMeBackHandler();
	markAllCallMeBackAsRead();
	...
	jQuery.ajax({
		...
		success: function(data){
			...
			if(status == 'no_authenticated'){
				...
				bootbox.alert("Phải đăng nhập để sử dụng các gói tin VIP của Nhà Tìm Chủ.", function(r){
					if(!r){
						$('#sl_package').val('standard');
						$("#packagePrice").html(0);
					}
...
function updateCallMeBackHandler(){
	$('.read-callmeback').click(function(){
		var callMeBackID = $(this).data('callmebackid');
		$("#callMeBackID").val(callMeBackID);
		$("#crudaction").val("update");
		$("#frmCallMeBack").submit();
	});
}

function markAllCallMeBackAsRead(){
	$("#btnMarkAllAsRead").click(function(){
		$("#crudaction").val("update-all");
		$("#frmCallMeBack").submit();
	})
}
